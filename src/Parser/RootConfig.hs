...
languagesParser value =
  case value of
    (JsonTypes.Array v) ->
      let objectsList =
            traverse (toObject InvalidLangaugesChildType) $ Vector.toList v
      in objectsList >>=
         (Misc.mapLeft InvalidLangauges .
          traverse LanguageConfigParser.parseJsonObject)
...
typesParser value =
  case value of
    (JsonTypes.Array v) ->
      let objectsList =
            traverse (toObject InvalidTypesChildType) $ Vector.toList v
      in objectsList >>=
         (Misc.mapLeft InvalidTypes . traverse CustomTypeParser.parseJsonObject)
    invalid -> Left (InvalidTypesType invalid)

toObject ::
     (JsonTypes.Value -> ParseError)
  -> JsonTypes.Value
  -> Either ParseError JsonTypes.Object
